...
const Component = ({ data, value }) => {
  ...
  return (
    <div
      ref={ref}
      className={`builder-view-component ${
        data?.body?.setting?.fullWidth ? "container-fluid" : "container"
      }`}
    >
...
const Row = ({ data, path }) => {
  ...
  const widthData = data.children.map((item) =>
    item.children.map((test) => test.body.setting.fullWidth)
  );
