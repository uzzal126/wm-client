...
import { TypeAddressField } from "./Type";
...
export default function AddressField({
  ...
  useEffect(() => {
    if (
      ...
      setData !== null &&
      Object.keys(setData).length &&
      setData.area_id !== 0 &&
      setData.area_id !== selected.area_id
    ) {
      getArea(setData);
  ...
  const handleClickSearchVal = (item: any) => {
    formik.setFieldValue("region", {
      id: item?.region_id,
      name: item?.region_name,
    });
    formik.setFieldValue("city", { id: item?.city_id, name: item?.city_name });
    formik.setFieldValue("zone", { id: item?.zone_id, name: item?.zone_name });
    formik.setFieldValue("area", { id: item?.area_id, name: item?.area_name });
  };
