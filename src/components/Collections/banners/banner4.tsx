...
const dummy = "https://dummyimage.com/650x400/d1d1d1/222";
...
const MasterCollection: FC<iProps> = ({ item, col = "6" }) => {
  let storeData = useSelector(selectStoreData);
  const redirect_url = getBannerRdirectURL(item, storeData?.menu_all || []);
  const dImage = imageDimentionHelper(item?.banner_url, 650, 400, true);
...
const HomeBanner4: FC<Props> = ({ data }) => {
  let customSettings = {
    ...
    initialSlide: 0,
    rows: 1,
    nextArrow: <BsFillArrowRightCircleFill size={30} />,
    prevArrow: <BsArrowLeftCircleFill size={30} />,
    responsive: [
      {
        breakpoint: 768,
        settings: {
